...
public class SenderUtility {
    ...
    public void send(){
       ...
       // template.convertAndSend("telangana", vineelMsg);
        ...
        String  text=text(vineelMsg);
        template.convertAndSend("telangana",text);

    }

    String text(SimpleMessage message){
        String text=message.getReceiverName()+"-"+message.getMessageText();
        return text;
    }
}
